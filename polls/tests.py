...
class PollDetailViewTests(TestCase):
    def test_detail_view_with_one_poll1(self):
        """
        The detail view should return status 200 if a poll exists
        """
        question = "Am I poll 1"
        create_poll(question=question, days=-1)
        current_poll = Poll.objects.get(question=question)
        create_fiber_page_tree()
        response = self.client.get(reverse('polls:detail', args=[current_poll.id]))
        self.assertEqual(response.status_code, 200)
