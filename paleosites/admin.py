    ...
    fieldsets = [
              ('Sample Information', {
                 'fields': [('layer','industry','industry_2','industry_3'),
                            ('cat_no','sample','technique','date','sd_plus','sd_minus','corrected_date_BP','plus','minus','intcal09_max','intcal09_min'),
...
class Site_plus_dates_admin(admin.ModelAdmin):
    ...
    fieldsets = [
              (None, {
                 'fields': [('site','site_type'),('latitude','longitude','altitude','country'),('data_source')] } ) ]
...
class Site_admin(admin.OSMGeoAdmin):
    fieldsets = [
              (None, {
                 'fields': [('site','site_type','data_source','display'),('latitude','longitude','altitude','country'),('map_location','notes')] } ) ]
    readonly_fields = ('latitude','longitude',)
    list_display = ('site','site_type','latitude','longitude','altitude','country','data_source')
        ...
        with open('D:/Users/mcpherro/PycharmProjects/Sites/sites.csv', 'rb') as f:
            ...
            for row in reader:
                ...
                for c in countries:
                    if row[2]==c[1]:
                        country_name = c[0]
                s = Site(id=row[0],site=row[1],country=country_name,data_source=row[3],site_type=row[7],display=row[8])
                if row[4]!='NA':
                    s.latitude = row[4]
                if row[5]!='NA':
                    s.longitude = row[5]
...
class Date_admin(admin.ModelAdmin):
    ...
    def import_dates(modeladmin, request, queryset):
        with open('D:/Users/mcpherro/PycharmProjects/Sites/dates.csv', 'rb') as f:
            reader = csv.reader(f)
            for row in reader:
                d = Date(id=row[0],site_id=row[1],layer=row[2],industry=row[3],industry_2=row[4],industry_3=row[5],cat_no=row[6],sample=row[10],technique=row[11],hominid_remains=row[15],bibliography=row[16],period=row[17],notes=row[18])
                if row[7]!='NA':
                    d.date=row[7]
                if row[8]!='NA':
                    d.sd_plus=row[8]
